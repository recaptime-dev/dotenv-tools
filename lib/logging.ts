import { loggerPrefix } from "./constants.ts";

import { getColor, bold } from "./helpers/colors.ts";
...
export function formatMessage(level: string, message: string) {
  ...
  switch (level.toLowerCase()) {
    ...
    case "errorvp":
      return error(
        `[${loggerPrefix}][precommit] ${formattedMessage}`
      );
    case "errorvpb":
      return error(
        `[${loggerPrefix}][prebuild] ${formattedMessage}`
      );
    case "errornocolor":
